pipeline{ 
  ...
  steps {
    script {
      ...
      bat "docker ps"
      bat "docker run --network some-network -e DOCKER_TLS_CERTDIR=/certs -v some-docker-certs-client:/certs/client:ro -v C:/Jenkins/Agent/workspace/CI_Quarkus/:/datos docker:latest sh -c 'cd /datos && docker build -t devops-quarkus:latest -f src/main/docker/Dockerfile.jvm .'"
      bat "docker ps"
    }
  }
}
